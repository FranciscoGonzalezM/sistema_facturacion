...
@register.simple_tag(takes_context=True)
def has_org_role(context, roles):
    """Return True only if the current user has one of the roles in the current organisation.

    Unlike `is_org_role`, this does NOT auto-allow staff users. Use when you need to check
    a specific role (for example to hide UI from `cajero`).
    Usage: {% has_org_role 'cajero' as is_cajero %}
    """
    request = context.get('request')
    if not request:
        return False
